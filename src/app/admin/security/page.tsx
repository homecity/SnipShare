        ...
        {loading ? (
          ...
          <>
            ...
            {tab === 'activity' && (
              <div className="bg-slate-800/30 border border-slate-700/50 rounded-xl overflow-hidden">
                <div className="px-4 py-3 border-b border-slate-700/50">
                  <h3 className="text-white font-medium">IP Activity (Last 24 Hours)</h3>
                </div>
                {ipActivity.length === 0 ? (
                  <div className="p-8 text-center text-slate-400">No activity in the last 24 hours</div>
                ) : (
                        ...
                        {ipActivity.map((item, i) => (
                          <tr key={i} className="border-b border-slate-700/30 hover:bg-slate-700/20">
                            ...
                            <td className="px-4 py-3">
                              <span className={`font-medium ${item.count >= 10 ? 'text-red-400' : item.count >= 5 ? 'text-yellow-400' : 'text-green-400'}`}>
                                {item.count}
                              </span>
                            </td>
                            <td className="px-4 py-3 text-slate-400">{formatTime(item.lastActivity)}</td>
                            <td className="px-4 py-3">
                              <button
                                onClick={() => handleBlock(item.ip, `High activity: ${item.count} requests`)}
                                disabled={actionLoading}
                                className="px-3 py-1 bg-red-500/20 hover:bg-red-500/40 text-red-300 rounded text-xs transition"
                              >
                                Block
                              </button>
                            </td>
            ...
            {tab === 'blocked' && (
              <div className="space-y-4">
                ...
                <div className="bg-slate-800/30 border border-slate-700/50 rounded-xl p-4">
                  <h3 className="text-white font-medium mb-3">Block IP Address</h3>
                  <div className="flex gap-2 flex-wrap">
                    <input
                      ...
                      className="flex-1 min-w-[200px] px-4 py-2 bg-slate-900/50 border border-slate-600 rounded-lg text-white placeholder-slate-500 text-sm focus:outline-none focus:ring-2 focus:ring-purple-500"
                    />
                    <input
                      type="text"
                      value={blockReason}
                ...
                <div className="bg-slate-800/30 border border-slate-700/50 rounded-xl overflow-hidden">
                  <div className="px-4 py-3 border-b border-slate-700/50">
                    <h3 className="text-white font-medium">Blocked IPs ({blockedIps.length})</h3>
                  </div>
                  {blockedIps.length === 0 ? (
                    <div className="p-8 text-center text-slate-400">No blocked IPs</div>
                  ) : (
                    <div className="overflow-x-auto">
                      <table className="w-full text-sm">
                        <thead>
                        ...
                        <tbody>
                          {blockedIps.map(item => (
                            <tr key={item.id} className="border-b border-slate-700/30 hover:bg-slate-700/20">
                              ...
                              <td className="px-4 py-3 text-slate-400">{new Date(item.blocked_at * 1000).toLocaleString()}</td>
                              <td className="px-4 py-3">
                                <button
                                  onClick={() => handleUnblock(item.ip)}
                                  disabled={actionLoading}
                                  className="px-3 py-1 bg-green-500/20 hover:bg-green-500/40 text-green-300 rounded text-xs transition"
                                >
                                  Unblock
                                </button>
                              </td>
            ...
            {tab === 'logs' && (
              <div className="bg-slate-800/30 border border-slate-700/50 rounded-xl overflow-hidden">
                <div className="px-4 py-3 border-b border-slate-700/50">
                  <h3 className="text-white font-medium">Rate Limit Logs (Last 24 Hours, max 100)</h3>
                </div>
                {rateLogs.length === 0 ? (
                  <div className="p-8 text-center text-slate-400">No rate limit logs</div>
                ) : (
                      ...
                      <tbody>
                        {rateLogs.map(log => (
                          <tr key={log.id} className="border-b border-slate-700/30 hover:bg-slate-700/20">
                            <td className="px-4 py-2 text-slate-400 whitespace-nowrap">{formatTime(log.timestamp)}</td>
                            <td className="px-4 py-2 text-white font-mono">{log.ip}</td>
